...
/* Check if register `r` belongs to any currently live local variable. */
static bool is_local_reg(uint8_t r) {
    for (int i = (int)rc->local_count - 1; i >= 0; i--) {
        if (rc->locals[i].reg == r) return true;
    }
    return false;
}
...
static void compile_expr(const Expr *e, uint8_t dst, int line) {
    ...
    case EXPR_BINOP: {
        ...
        /* General binary: compile left into lhs_reg, right into rhs_reg.
         * If dst is a local register, compiling the left into dst would clobber
         * the local before the right operand can read it (register aliasing).
         * Use a temporary for the left operand in that case. */
        uint8_t lhs;
        bool lhs_is_tmp = false;
        if (is_local_reg(dst)) {
            lhs = alloc_reg();
            lhs_is_tmp = true;
        } else {
