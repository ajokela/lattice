...
static char *hex_encode(const unsigned char *hash, unsigned int len) {
    ...
    if (!hex) return NULL;
    for (unsigned int i = 0; i < len; i++) { snprintf(hex + i * 2, 3, "%02x", hash[i]); }
    hex[len * 2] = '\0';
    return hex;
}
...
char *crypto_hmac_sha256(const char *key, size_t key_len, const char *data, size_t data_len, char **err) {
    unsigned char result[EVP_MAX_MD_SIZE];
    unsigned int result_len = 0;
    unsigned char *r =
        HMAC(EVP_sha256(), key, (int)key_len, (const unsigned char *)data, data_len, result, &result_len);
    if (!r) {
...
static void pc_sha256_compress(pc_sha256_ctx *c, const uint8_t block[64]) {
    ...
    for (int i = 16; i < 64; i++) w[i] = S256_s1(w[i - 2]) + w[i - 7] + S256_s0(w[i - 15]) + w[i - 16];

    uint32_t a = c->state[0], b = c->state[1], cc = c->state[2], d = c->state[3], e = c->state[4], f = c->state[5],
...
static void pc_sha256_final(pc_sha256_ctx *c, uint8_t hash[32]) {
    ...
    for (int i = 0; i < 8; i++) c->buffer[56 + i] = (uint8_t)(c->bitcount >> (56 - i * 8));
    pc_sha256_compress(c, c->buffer);
    for (int i = 0; i < 8; i++) {
...
static const int md5_s[64] = {7,  12, 17, 22, 7,  12, 17, 22, 7,  12, 17, 22, 7,  12, 17, 22, 5,  9,  14, 20, 5,  9,
...
static void pc_md5_final(pc_md5_ctx *c, uint8_t hash[16]) {
    ...
    for (int i = 0; i < 8; i++) c->buffer[56 + i] = (uint8_t)(c->bitcount >> (i * 8));
    pc_md5_compress(c, c->buffer);
    for (int i = 0; i < 4; i++) {
...
static void pc_sha512_compress(pc_sha512_ctx *c, const uint8_t block[128]) {
    ...
    for (int i = 16; i < 80; i++) w[i] = S512_s1(w[i - 2]) + w[i - 7] + S512_s0(w[i - 15]) + w[i - 16];

    uint64_t a = c->state[0], b = c->state[1], cv = c->state[2], d = c->state[3], e = c->state[4], f = c->state[5],
             g = c->state[6], h = c->state[7];
    for (int i = 0; i < 80; i++) {
...
static void pc_sha512_final(pc_sha512_ctx *c, uint8_t hash[64]) {
    ...
    for (int i = 0; i < 8; i++) c->buffer[112 + i] = (uint8_t)(c->bitcount[1] >> (56 - i * 8));
    for (int i = 0; i < 8; i++) c->buffer[120 + i] = (uint8_t)(c->bitcount[0] >> (56 - i * 8));
    pc_sha512_compress(c, c->buffer);
    for (int i = 0; i < 8; i++) {
...
uint8_t *crypto_random_bytes(size_t n, char **err) {
    ...
    if (!f) {
        free(buf);
        *err = strdup("random_bytes: cannot open /dev/urandom");
        return NULL;
    }
    if (fread(buf, 1, n, f) != n) {
        fclose(f);
        free(buf);
        *err = strdup("random_bytes: short read from /dev/urandom");
        return NULL;
    }
    fclose(f);
    return buf;
...
char *crypto_base64_decode(const char *data, size_t len, size_t *out_len, char **err) {
    ...
    if (len == 0) {
        char *out = malloc(1);
        if (!out) {
            *err = strdup("base64_decode: out of memory");
            return NULL;
        }
        out[0] = '\0';
        *out_len = 0;
    ...
    for (size_t i = 0; i < len; i += 4) {
        ...
        unsigned int n = ((unsigned int)a << 18) | ((unsigned int)b << 12) | ((unsigned int)cv << 6) | (unsigned int)dv;
