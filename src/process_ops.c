...
static void *posix_write_worker(void *opaque) {
    ...
    /* Darwin can route a pipe's SIGPIPE to another unblocked thread even when
     * the writing thread blocks it. Disable generation on this descriptor;
     * write still reports EPIPE, and the setting does not change process-wide
     * signal disposition or leak into unrelated embedding threads. */
    if (fcntl(worker->fd, F_SETNOSIGPIPE, 1) != 0) {
        worker->io_error = errno;
        close(worker->fd);
        worker->fd = -1;
        return NULL;
     ...
     * Preserve the caller's mask and any SIGPIPE that was already pending: the
     * EPIPE write can leave a new SIGPIPE pending on this worker, and Darwin
     * may deliver that signal to another thread when the worker exits. */
    sigset_t sigpipe;
    sigset_t previous_mask;
    sigset_t pending;
    sigemptyset(&sigpipe);
    sigaddset(&sigpipe, SIGPIPE);
    int mask_error = pthread_sigmask(SIG_BLOCK, &sigpipe, &previous_mask);
    if (mask_error != 0) {
    ...
    if (sigpending(&pending) != 0) {
        ...
        close(worker->fd);
        worker->fd = -1;
        int restore_error = pthread_sigmask(SIG_SETMASK, &previous_mask, NULL);
        if (worker->io_error == 0 && restore_error != 0) worker->io_error = restore_error;
        return NULL;
    }
    ...
    while (written < worker->len) {
        ...
        if (count < 0 && errno == EPIPE) {
            saw_epipe = true; /* child deliberately stopped reading */
            break;
        }
    ...
    /* Consume only the SIGPIPE generated by our EPIPE write. If SIGPIPE is
     * ignored process-wide (as in the C test runner), no signal will be
     * pending and sigwait must not be called. */
    if (saw_epipe && !sigpipe_was_pending) {
        if (sigpending(&pending) != 0) {
            if (worker->io_error == 0) worker->io_error = errno;
        } else if (sigismember(&pending, SIGPIPE) == 1) {
            int caught_signal = 0;
            int wait_error = sigwait(&sigpipe, &caught_signal);
            if (wait_error != 0 && worker->io_error == 0) worker->io_error = wait_error;
        }
    }
