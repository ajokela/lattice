...
static const LspBuiltinDoc builtin_docs[] = {
    ...
    {"exec", "exec(cmd: String) -> String", "Runs a command through the platform shell and returns stdout."},
    {"exec_argv", "exec_argv(program: String, args: [String], stdin: String|Nil, options: Map|Nil = nil) -> Map",
     "Runs a program directly, optionally bounded by timeout and output limits."},
    {"shell", "shell(cmd: String) -> Map",
     "Runs a command through the platform shell and returns {exit_code, stdout, stderr}."},
