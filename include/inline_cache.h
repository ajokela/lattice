...
static inline void pic_table_init(PICTable *t) {
    t->slots = NULL;
}
...
static inline PICSlot *pic_table_ensure(PICTable *t) {
    if (!t->slots) {
        t->slots = (PICSlot *)calloc(PIC_DIRECT_SLOTS, sizeof(PICSlot));
    }
    return t->slots;
}
...
static inline uint16_t pic_lookup(const PICSlot *slot, uint8_t type_tag, uint32_t mhash) {
    for (int i = 0; i < slot->count; i++) {
        if (slot->entries[i].type_tag == type_tag &&
            slot->entries[i].method_hash == mhash) {
